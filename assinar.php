...
if (!$token) {
    die("Token do documento não informado.");
}
...
if (!$doc) {
    die("Documento inválido ou não encontrado.");
}
...
if (isset($_POST['assinar']) && !empty($_POST['assinatura_data'])) {
    ...
    $stmt = $pdo->prepare("
        UPDATE documentos 
        SET assinatura_cliente = ?, status = 'assinado_cliente' 
        WHERE token = ?
    ");
...
GLOW<span class="text-indigo-500">PDF</span>
</h1>

<span class="text-[10px] bg-indigo-500/20 text-indigo-400 px-3 py-1 rounded-full font-bold uppercase">
Assinatura Digital
</span>

</div>

<h2 class="text-lg font-bold text-white mb-2 uppercase italic">
📄 Documento para: <?= htmlspecialchars($doc['cliente']) ?>
</h2>
...
<label class="block text-center font-bold text-[10px] mb-3 text-indigo-400 uppercase tracking-widest">
Use o dedo ou mouse para assinar abaixo ✍️
</label>
...
<button type="button"
onclick="limpar()"
class="w-1/4 bg-slate-800 text-slate-400 py-4 rounded-2xl font-bold text-xs uppercase hover:bg-slate-700 transition">
Limpar
</button>
...
name="assinar"
onclick="salvar()"
class="w-3/4 bg-indigo-600 py-4 rounded-2xl font-black text-white uppercase tracking-widest hover:bg-indigo-500 shadow-lg shadow-indigo-900/40 transition">
...
O documento foi enviado de volta para a empresa para finalização oficial.
</p>
